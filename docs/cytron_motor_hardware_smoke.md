...
Arduino UNO bridge → Cytron MDDS30 (SmartDriveDuo-30) → two brushed DC motors. The bridge sketch and
its wiring/DIP-switch reference live in the
[Cytron Motordriver Tester](https://github.com/michal-kukucka/cytrontester) repository
(`arduino/mdds30_bridge/mdds30_bridge.ino`). Default CI stays hardware-free; run these steps only
with the robot safely lifted or mechanically disabled.

## Preconditions

- Build with `ROZETA_WITH_SERIAL_MOTORS=ON`.
- Upload the `mdds30_bridge` sketch to the Arduino UNO and confirm it prints
  `OK HERMES_MDDS30_BRIDGE_READY` on boot with motors stopped.
...
4. Verify the application resends the last safe command at `cytron_repeat_interval` (100 ms) using
   `MissionRuntime` motor keepalive, not a serial backend thread. Pause the sender and confirm the
   bridge prints `OK WATCHDOG_STOP` and both motors stop within ~300 ms.
...
6. Exercise a `SpeedRamp` accelerate/decelerate cycle and confirm smooth linear response ending in
   `STOP`.
7. Record bridge device path, observed wheel directions, and any wiring inversion in the field log.
